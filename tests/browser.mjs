...
try{
  ...
  await page.locator('[data-shipment="demo"]').click();await page.locator('[data-step="1"]').first().click();await page.locator('#open-cart').click();await page.locator('#submit-reserve').click();
  ...
  const exported=await JSZip.loadAsync(await readFile('test-results/reservations.xlsx'));assert(exported.file('xl/worksheets/sheet1.xml'));
